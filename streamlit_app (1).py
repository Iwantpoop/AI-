...
#         st.error(f"S&P500 종목을 가져오는 데 실패했습니다: {e}. 기본 종목을 사용합니다.")
...
#         st.error("NASDAQ100 테이블에서 'Ticker' 컬럼을 찾을 수 없습니다. 기본 종목을 사용합니다.")
#         return ["AAPL", "MSFT", "NVDA", "AMZN", "META", "TSLA"]
#     except Exception as e:
#         st.error(f"NASDAQ100 종목을 가져오는 데 실패했습니다: {e}. 기본 종목을 사용합니다.")
#         return ["AAPL", "MSFT", "NVDA", "AMZN", "META", "TSLA"]
# 
# @st.cache_data
...
#                 return table["Symbol"].tolist()
#         st.error("DOW30 테이블에서 'Symbol' 컬럼을 찾을 수 없습니다. 기본 종목을 사용합니다.")
#         return ["AAPL", "MSFT", "JPM", "V", "UNH", "HD", "PG"]
#     except Exception as e:
#         st.error(f"DOW30 종목을 가져오는 데 실패했습니다: {e}. 기본 종목을 사용합니다.")
#         return ["AAPL", "MSFT", "JPM", "V", "UNH", "HD", "PG"]
# 
# # ----------------------------------
# # 🔹 시장 선택 (사이드바)
...
#     strong = []
#     # 성능을 위해 처음 50개 종목만 분석
#     for ticker in market_tickers[:50]:
#         try:
#             data = yf.download(ticker, period="6mo", progress=False)
#             if len(data) < 60: # MA 계산에 충분한 데이터 확인
#                 continue
...
#     st.sidebar.info("강세 종목 필터링 결과가 적어, 다른 지수 내 종목들을 포함합니다.")
#     temp_list = list(strong_tickers_list)
#     added_count = 0
#     for t in all_market_tickers:
#         if t not in temp_list and added_count < 10: # 최대 10개 추가 종목
#             temp_list.append(t)
#             added_count += 1
#     if not temp_list: # 최종 폴백 (모든 시도 실패 시)
...
# # 🔹 강세 종목 선택 (사이드바)
# # ----------------------------------
# selected_stock_ticker = st.sidebar.selectbox("강세 종목 선택", strong_tickers_list)
# 
# # ----------------------------------
# # 🔹 데이터 로드 (선택된 종목)
# # ----------------------------------
...
# # 📊 지표 UI 표시 (선택된 종목)
# # ----------------------------------
# st.subheader(f"📊 {selected_stock_ticker} 핵심 지표")
# 
# col1, col2, col3 = st.columns(3)
# col4, col5, col6 = st.columns(3)
# 
# col1.metric("현재가", f"{current_price:.2f}")
# col2.metric("상승 확률", f"{up_prob*100:.2f}%")
# col3.metric("모델 정확도", f"{accuracy*100:.2f}%")
# 
# col4.metric("전략 수익률 (2년)", f"{strategy_return:.2f}%")
# col5.metric("시장 수익률 (2년)", f"{market_return:.2f}%")
# col6.metric("이상적 보유기간", f"{ideal_day}일")
# 
# st.markdown(
#     f"📌 보유기간 후 예상 주가: **{ideal_price:.2f}**  \n"
#     f"📈 기대 수익률: **{ideal_return:.2f}%**"
# )
# 
# # ----------------------------------
# # 📈 차트 (선택된 종목)
# # ----------------------------------
# st.subheader(f"📈 {selected_stock_ticker} 가격 & 예측")
# 
